...
for x in team_list:
    print('チーム:{}'.format(x))
    # ディレクトリ：dailyを作成
    daily_dir = r'{0}\{1}\2019-20_regular\daily'.format(division,x)
    if not os.path.isdir(daily_dir):
        os.makedirs(daily_dir)
        
    # ファイル名だけを取得
    path = r'{0}\{1}\2019-20_regular'.format(division,x)
    files = os.listdir(path)
    files_list = [f for f in files if os.path.isfile(os.path.join(path, f))]
    # print(files_file)
    data_list = []
    for i in files_list:
        data = pd.read_csv('{0}/{1}/2019-20_regular/{2}'.format(division,x,i), encoding='cp932')
    ...
    for i in range(len(player_list)):
        ...
        daily_stats.to_csv('{0}/{1}/2019-20_regular/daily/{2}_{3}_dailystats.csv'.format(division,x,number_list[i],player_list[i]), index=False, encoding='cp932')
